...
namespace WinFormsApp6
{
    ...
    class SimpleConfigModule : NinjectModule
    {
        ...
        {
            Bind<IRepository<Entities>>().ToMethod(ctx => // связывает dapperrepozitory и irepozitory и подписывает на события
            {
                string connectionString = "Data Source=database.db"; 
                return new DapperRepozitory<Entities>(connectionString);// возвращаем новый объект класса DapperRepozitory, который будет работать с типом данных Employee, connectionString передается в конструктор
